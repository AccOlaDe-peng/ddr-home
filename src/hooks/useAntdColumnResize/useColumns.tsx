...
import { INTERNAL_KEY } from "./contant";
...
const useColumns = ({
  ...
  const calculateWidth = useCallback(
    (column: Column): number => {
      ...
      let totalWidth = isLeaf
        ? 0
        : Number(column?.width ?? Number(minWidth + 15));
      if (isLeaf) {
        totalWidth = +(column?.children as [])?.reduce(
  ...
  const tableWidth = useMemo(() => {
    return reduce(
      resizableColumns,
      (sum, column) => sum + calculateWidth(column),
      34
    );
  }, [resizableColumns, calculateWidth]);

  const handleResizableColumns = useCallback(
    (key: string | number, interWidth: number) => {
      columnMap.set(key, interWidth);
      setResizableColumns((prev) =>
        prev?.map((column) => updateResizableColumns(column, key, interWidth))
      );
    },
    []
  );
    ...
    (columns: Column[]): Column[] => {
      return columns?.map((column, index: number) => {
        ...
        const width = column?.width
          ? Number(column?.width) + deviationWidth
          : column?.width;
        return {
          ...column,
  ...
  const updateResizableColumns = useCallback(
    <T extends Column>(
      column: T,
      key: string | number,
      interWidth: number
      ...
      if (!isTarget && Array.isArray(column.children)) {
        column.children = column.children.map((item) =>
          updateResizableColumns(item, key, interWidth)
        );
      }
      return {
        ...column,
  ...
  const mergedColumns = (
    ...
    if (
      resizableColumns === initialColumns ||
      !resizableColumns ||
    ...
    return initialColumns.map((item) => {
      const key =
        (item[INTERNAL_KEY] as string | number | undefined) || item.key;
